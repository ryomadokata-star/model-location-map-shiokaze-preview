...
async function importEventUrl(url){const target=url||$("#eventUrl")?.value||"https://www.fresh-club.net/outdoor/detail/1553";$("#importStatus")&&($("#importStatus").textContent="\u53d6\u5f97\u4e2d...");try{const r=await fetch("/api/import?url="+encodeURIComponent(target));const data=await r.json();if(!r.ok)throw new Error(data.error||"\u53d6\u5f97\u306b\u5931\u6557\u3057\u307e\u3057\u305f");if(!data.models?.length)return;models=data.models.map((m,i)=>Object.assign({},m,{id:"fresh"+i,status:i%3===0?"shooting":i%3===1?"break":"preparing",publishStatus:m.publishStatus||"confirm",mapX:[22,48,74,28,55,80,23,52,78][i%9],mapY:[24,23,25,48,45,50,72,70,74][i%9],parts:m.parts?.length?m.parts:["1\u90e8"]}));selected=models[0].id;render();const title=data.event?.title||"Fresh! \u5c4b\u5916\u64ae\u5f71\u4f1a";const venue=data.event?.venue||"\u4f1a\u5834\u672a\u8a2d\u5b9a";document.querySelector(".topbar .eyebrow").textContent=title+" / "+venue;let label=$("#venueLabel");if(!label){label=document.createElement("div");label.id="venueLabel";label.className="area";label.style.cssText="display:block!important;left:16px;top:48px;background:rgba(255,255,255,.86);color:#274334;font-weight:900";$("#venueMap")?.append(label)}label.textContent=venue+" MAP";$("#importStatus")&&($("#importStatus").textContent=data.models.length+"\u540d\u3092\u53d6\u5f97 / \u4f1a\u5834: "+venue)}catch(e){$("#importStatus")&&($("#importStatus").textContent=e.message||"\u53d6\u5f97\u306b\u5931\u6557\u3057\u307e\u3057\u305f")}}
$("#parseEvent")?.addEventListener("click",()=>importEventUrl($("#eventUrl")?.value));$("#eventUrl")?.addEventListener("change",()=>importEventUrl($("#eventUrl")?.value));
